...
from stepik.schemas import Product as ProductSchema, ProductCreate, Review
from stepik.db_depends import get_db, get_async_db
from sqlalchemy.ext.asyncio import AsyncSession

from stepik.models.users import User as UserModel
from stepik.auth import get_current_seller
...
async def get_product(product_id: int, db: AsyncSession = Depends(get_async_db)):
    """
    Возвращает отзыв о товаре по его ID.
    """
    ...
    review_db = await db.scalars(select(ReviewModel).where(ReviewModel.product_id == product_id, ReviewModel.is_active == True))

    return review_db.all()
